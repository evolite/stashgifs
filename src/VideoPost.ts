...
export class VideoPost extends BasePost {
  ...
  private getPreviewPosterUrl(): string | undefined {
    ...
    const scenePreview = m?.scene?.paths?.preview;
    if (scenePreview) {
      return this.buildUrlWithCacheBusting(scenePreview);
    }
    const sceneScreenshot = m?.scene?.paths?.screenshot;
    if (sceneScreenshot) {
      return this.buildUrlWithCacheBusting(sceneScreenshot);
    }
    return undefined;
  }

  private getPreferredPosterUrl(): string | undefined {
    if (this.isReelMode) {
      return this.getPreviewPosterUrl();
    }
  ...
  private createRatingSection(): HTMLElement {
    if (!this.ratingControl) {
      ...
      this.ratingControl = new RatingControl({
        ...
        getRating100: () => this.data.marker.scene.rating100,
        onUpdateRating100: (value) => {
          this.data.marker.scene.rating100 = value;
        },
        onSaveRating10: api
          ? (rating10) => api.updateSceneRating(this.data.marker.scene.id, rating10)
          : undefined,
  ...
  private validateAndPrepareUpgrade(): { sceneVideoUrl: string; playerContainer: HTMLElement; wasPlaying: boolean } {
    ...
    const normalizedSceneVideoUrl = normalizeMediaUrl(sceneVideoUrl);
    if (!normalizedSceneVideoUrl) {
  ...
  loadPlayer(videoUrl: string, startTime?: number, endTime?: number): NativeVideoPlayer | undefined {
    ...
    try {
      ...
      this.player = new NativeVideoPlayer(playerContainer, normalizedVideoUrl, {
        muted: shouldBeMuted,
        autoplay: false,
  ...
  hasVideoSource(): boolean {
    return !!normalizeMediaUrl(this.data.videoUrl);
  }
